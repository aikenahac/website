...
export async function getRecentlyPlayed(access_token: string) {
  ...
  const recentArrRes = res.body
    ? (await res.json()).items.map((item: any) => item.track)
    : undefined;
  // const parsed = queueFirst ? songResponseSchema.safeParse(queueFirst) : null;
  const recentTracksRes: SongResponse[] = recentArrRes.map(
    (item: any) => songResponseSchema.safeParse(item).data,
  );

  const recentlyPlayed: SpotifySong[] | undefined =
    recentTracksRes && recentTracksRes.length > 0
      ? recentTracksRes.map((track) => ({
          title: track.name,
